...
export class SecureComponent implements OnInit {
  ...
  loadChat() {
    ...
    if (!this.userData.admin && this.userData.domain != 'dev-srore.myshopify.com') {
      window.$crisp = []; window.CRISP_WEBSITE_ID = "e88807ec-c2cd-4778-91f4-99a155c5acf4";
      this.addJsToElement('https://client.crisp.chat/l.js').onload = () => {
        ...
        let tmpProductCount = this.userData.productCount ? this.userData.productCount : 0;
        window.$crisp.push(["set", "session:segments", [[this.userData.recurringPlanName, this.userData.shopUrl, tmpProductCount.toString(),], true]])
